...
export default function ProductPage() {
  ...
  const images = (product.images && product.images.length > 0)
    ? product.images
    : Array.from({ length: 4 }, (_, i) => ({
        image_url: `https://picsum.photos/seed/${product.id + i * 7}/800/800`,
        alt_text:  product.name,
        is_primary: i === 0,
      }));

  const mainSrc = images[activeImg]?.image_url
          ...
          <div>
            ...
            <div className="relative rounded-2xl overflow-hidden bg-masa-light aspect-square mb-4 shadow-sm group">
              <img
                ...
                onError={e => { e.target.src = `https://picsum.photos/seed/${product.id + 99}/800/800`; }}
              />
              {/* prev / next arrows */}
              {images.length > 1 && (
                <>
                ...
                {images.map((img, i) => (
                  ...
                  >
                    <img
                      ...
                      onError={e => { e.target.src = `https://picsum.photos/seed/${product.id + i + 5}/200/200`; }}
                    />
                  </button>
                ))}
              </div>
