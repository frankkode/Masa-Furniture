  ...
  return (
    <span className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map(n => (
        <svg
          key={n}
          className={`w-3.5 h-3.5 ${n <= stars ? 'text-masa-accent' : 'text-gray-300'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
      ...
      {count > 0 && <span className="text-xs text-masa-gray ml-1">({count})</span>}
    </span>
  );
}
...
function ProductCard({ product }) {
  ...
  const imgSrc = product.image_url || `https://picsum.photos/seed/${product.id}/400/320`;

  return (
    <div className="card group flex flex-col overflow-hidden">
      {/* image */}
      <Link to={`/product/${product.id}`} className="block overflow-hidden bg-masa-light aspect-[4/3]">
        <img
          src={imgSrc}
          alt={product.name}
      ...
      <div className="p-4 flex flex-col flex-1">
        <p className="text-xs text-masa-gray capitalize mb-1">{product.category || 'Furniture'}</p>
        <Link
        ...
        <div className="mt-auto pt-3 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold bg-masa-dark text-white px-3 py-1.5 rounded-full
                       hover:bg-masa-accent transition-colors duration-200 disabled:opacity-60"
          >
            {adding ? (
