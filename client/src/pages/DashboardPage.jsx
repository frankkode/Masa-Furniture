...
/* ── SingleImageSlot ─────────────────────────────────────────
   One upload slot: file picker + drag-drop + URL fallback.
   Used inside MultiImageUploadField.
──────────────────────────────────────────────────────────── */
function SingleImageSlot({ value, onChange, label, isPrimary }) {
  ...
  return (
    <div className="border border-masa-border rounded-xl p-3 space-y-2">
      {/* slot header */}
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-masa-dark flex items-center gap-1.5">
          {isPrimary && <span className="text-[10px] bg-masa-accent text-white px-1.5 py-0.5 rounded-full">Primary</span>}
          {label}
        </span>
        <div className="flex gap-1">
          {[['upload','📁'],['url','🔗']].map(([id, icon]) => (
            <button key={id} type="button" onClick={() => setTab(id)}
              className={`text-[10px] px-2 py-0.5 rounded-full font-medium transition-colors
                ${tab === id ? 'bg-masa-accent text-white' : 'bg-masa-light text-masa-gray'}`}>
              {icon} {id === 'upload' ? 'Upload' : 'URL'}
            </button>
          ))}
        </div>
      </div>
      ...
      {value ? (
        <div className="flex items-center gap-2">
          <img src={value} alt="preview"
            ...
            className="w-16 h-16 object-cover rounded-lg border border-masa-border shrink-0"/>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-masa-gray truncate">{value}</p>
            <button type="button" onClick={() => onChange('')}
              className="mt-1 text-xs text-red-500 hover:text-red-700 font-medium">Remove</button>
          </div>
        </div>
      ) : tab === 'upload' ? (
        <label
          ...
          className={`flex flex-col items-center justify-center gap-1.5 w-full py-4 border-2 border-dashed rounded-xl
          ...
          {uploading ? (
            <svg className="w-5 h-5 animate-spin text-masa-accent" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
            </svg>
          ) : (
            <>
              <svg className="w-6 h-6 text-masa-gray" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
                  d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
              </svg>
              <p className="text-[10px] text-masa-gray">
                Drop or <span className="text-masa-accent font-semibold">browse</span>
              </p>
            </>
          )}
        ...
        <input type="text" value={value} onChange={e => onChange(e.target.value)}
          ...
          className="w-full px-3 py-2 text-xs border border-masa-border rounded-xl focus:outline-none focus:border-masa-accent"/>
      )}
...
/* ── MultiImageUploadField ────────────────────────────────────
   Up to 4 image slots. images = [{url, alt_text}]
   First slot is always the Primary image.
──────────────────────────────────────────────────────────── */
const MAX_IMAGES = 4;
const EMPTY_IMAGE = { url: '', alt_text: '' };

function MultiImageUploadField({ images, onChange }) {
  // pad to 4 slots (so we always show empty placeholders)
  const slots = [...images];
  while (slots.length < MAX_IMAGES) slots.push({ ...EMPTY_IMAGE });

  const updateSlot = (idx, url) => {
    const next = [...slots];
    next[idx] = { ...next[idx], url };
    // strip trailing empty slots
    let end = next.length;
    while (end > 0 && !next[end - 1].url) end--;
    onChange(next.slice(0, Math.max(1, end)));
  };

  return (
    <div className="space-y-3">
      <p className="text-xs font-semibold text-masa-dark">
        Product Images <span className="text-masa-gray font-normal">(up to {MAX_IMAGES} · first = primary)</span>
      </p>
      <div className="grid grid-cols-2 gap-3">
        {slots.map((slot, idx) => (
          <SingleImageSlot
            ...
            isPrimary={idx === 0}
            value={slot.url}
            onChange={url => updateSlot(idx, url)}
          />
        ))}
      </div>
    </div>
...
function ProductModal({ initial, categories, onSave, onCancel, saving }) {
  ...
  const [images, setImages] = useState([{ url: initial?.image_url || '', alt_text: '' }]);
  const [error, setError] = useState('');

  // When editing, load the full image list from the server
  useEffect(() => {
    if (!initial?.id) return;
    api.get(`/admin/products/${initial.id}`)
      .then(r => {
        const imgs = (r.data.images || []).map(i => ({ url: i.image_url, alt_text: i.alt_text || '' }));
  ...
  const handleSubmit = async (e) => {
    ...
    // pass images array (filter empty urls)
    ...
    try {
      await onSave({
        ...
        images:      validImages,
        // legacy field for table thumbnail display
...
function AdminProductsPanel() {
  ...
  const handleSave = async (data) => {
    setSaving(true);
    try {
      const primaryUrl = data.images?.[0]?.url || data.image_url || null;
      if (modal?.id) {
        const res = await api.patch(`/admin/products/${modal.id}`, data);
        setProducts(prev => prev.map(p =>
          p.id === modal.id ? { ...p, ...res.data.product, image_url: primaryUrl } : p
        ));
      } else {
